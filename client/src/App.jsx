...
function TaskRow({task,onOpen,onToggle}) { return <div className={`task-row ${task.completed_at?'completed':''}`} onClick={()=>onOpen(task.id)}><TaskCheckbox task={task} onToggle={onToggle}/><div className="task-content"><div className="task-title">{task.content}</div>{task.description&&<div className="task-description">{task.description}</div>}<TaskMeta task={task}/></div><div className="task-hover"><button><Edit3 size={16}/></button><button><MoreHorizontal size={17}/></button></div></div>; }

function ListView({tasks,sections,onOpen,onToggle,onInlineAdd,projectId,onCreateSection}) {
  const top=tasks.filter(t=>!t.parent_id);
  if(projectId&&sections.length){return <div className="sections-list">{sections.map(s=>{const st=top.filter(t=>t.section_id===s.id);return <section key={s.id} className="task-section"><h3>{s.name}<span>{st.length}</span></h3>{st.map(t=><TaskRow key={t.id} task={t} onOpen={onOpen} onToggle={onToggle}/>) }<InlineAdd onAdd={content=>onInlineAdd(content,s.id)}/></section>})}<section className="task-section"><h3>无分区</h3>{top.filter(t=>!t.section_id).map(t=><TaskRow key={t.id} task={t} onOpen={onOpen} onToggle={onToggle}/>)}<InlineAdd onAdd={c=>onInlineAdd(c,null)}/></section><button className="section-add" onClick={onCreateSection}><Plus size={15}/>添加分区</button></div>}
  return <div className="task-list">{projectId&&<button className="section-add" onClick={onCreateSection}><Plus size={15}/>添加分区</button>}{top.map(t=><TaskRow key={t.id} task={t} onOpen={onOpen} onToggle={onToggle}/>)}<InlineAdd onAdd={c=>onInlineAdd(c,null)}/>{!top.length&&<EmptyState/>}</div>;
}
...
function BoardView({tasks,sections,onOpen,onToggle,onInlineAdd}) {
  const cols=sections.length?sections:[{id:'none',name:'任务'}];
  return <div className="board">{cols.map(s=>{const items=tasks.filter(t=>!t.parent_id&&(s.id==='none'?!t.section_id:t.section_id===s.id));return <div className="board-column" key={s.id}><div className="board-title"><span>{s.name}</span><em>{items.length}</em><MoreHorizontal size={16}/></div>{items.map(t=><div className="task-card" key={t.id} onClick={()=>onOpen(t.id)}><div className="card-top"><TaskCheckbox task={t} onToggle={onToggle}/><span>{t.content}</span></div>{t.description&&<p>{t.description}</p>}<TaskMeta task={t}/></div>)}<InlineAdd onAdd={c=>onInlineAdd(c,s.id==='none'?null:s.id)}/></div>})}</div>;
}
  ...
  return <div className="drawer-shell"><div className="drawer-backdrop" onClick={onClose}/><aside className="task-drawer">
    ...
    {loading?<div className="drawer-loading"><Loader2 className="spin"/></div>:task&&<div className="drawer-content">
      ...
      <div className="drawer-side">
        <Field label="项目"><select value={task.project_id||''} onChange={e=>{const v=e.target.value||null;setTask({...task,project_id:v});save({project_id:v})}}><option value="">收件箱</option>{data.projects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></Field>
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content"><Header title={title} subtitle={subtitle} viewMode={viewMode} setViewMode={setViewOverride} onQuickAdd={()=>setQuickOpen(true)} onMenu={()=>setCollapsed(false)} sidebarCollapsed={collapsed} onNotifications={()=>setNotificationsOpen(!notificationsOpen)} unread={data.notifications.filter(n=>!n.read_at).length}/><div className={`content-body ${viewMode}`}>
      {viewMode==='list'&&<ListView tasks={visible} sections={active.type==='project'?data.sections.filter(s=>s.project_id===active.id):[]} onOpen={setTaskId} onToggle={toggle} onInlineAdd={inlineAdd} projectId={active.type==='project'?active.id:null} onCreateSection={createSection}/>} 
      {viewMode==='board'&&<BoardView tasks={visible} sections={active.type==='project'?data.sections.filter(s=>s.project_id===active.id):[]} onOpen={setTaskId} onToggle={toggle} onInlineAdd={inlineAdd}/>} 
